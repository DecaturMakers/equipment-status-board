...
- [ ] **Task 6: Wire the device selector into the template + live preview JS.**
  ...
  - Action:
    ...
    2. Add `img.onerror` / `img.onload` handlers that show/hide `#qr-preview-error` and hide/show the `<img>` accordingly, so a 400 preview response degrades to the readable message from Task 6.3 step 4 rather than a broken-image icon.
